...
contains	
  ...
  !------------------------------------------
	logical function has_boundary_effect(x,y,z,max_dist,printinfo)
		! Dummy
...
861		continue		
		...
		if(.true.) then
			if(trim(adjustl(gb_catalogue_name)).eq.'DR12v1-CMASS-S' &
				...
				if(gb_i_datatype.eq.1 .or. gb_i_datatype.eq.2 .or. gb_i_datatype.eq.3) then
...
!			write(98,*) iscan
			...
			if(gb_use_fixmd) then
				...
				max_dist = gb_fixmd
				if(has_boundary_effect(x,y,z,max_dist)) then
				...
				if(iscan.eq.check_iscan) then
					...
					tmplogical = has_boundary_effect(x,y,z,max_dist,.true.)
				endif
				if(has_boundary_effect(x,y,z,max_dist) .or. erflaglog) then
			...
			elseif(gb_consider_ran.eq.gb_radec3dran) then ! combination of 3d/radec random
				call xyz_to_radecr(x,y,z,ra,dec,r)
			...
			elseif(gb_consider_ran.eq.gb_noran) then
				erflagint = 0
				weirat = 1.0
			else
				print *, 'ERROR (grid_rho_drho_list)!!: Unknown value of gb_consider_ran: ', gb_consider_ran
				stop
			endif
