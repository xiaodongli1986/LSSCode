...
implicit none
	...
	character(len=char_len) :: tmpstr1, tmpstr2, inputfile, output_suffix, printstr, inputfiles(10000), headfile, outputfile
        ...
        real :: x,y,z,xyzmin, xyzmax, xyz_rescale,  xcut_min, xcut_max, ycut_min, ycut_max, zcut_min, zcut_max, Omega0, OmegaLambda, HubbleParam, redshift, Boxsize
	...
	printstr = "Usage: LSS_gadget_to_fmt3 -input inputfile -output output_suffix -redshift redshift -Omega0 Omega0 -OmegaLambda OmegaLambda -HubbleParam HubbleParam  -Boxsize Boxsize  -xyz_rescale xyz_rescale"
        numarg = iargc()
	if(numarg.le.1) then
		write(*,'(A)') printstr
		stop
...
100             exit
        ...
        do ifile = 1, nfile
                ...
                if(OmegaLambda.ne. -1.) then
                        if(ifile.eq.1) print *, '  ************** change OmegaLambda from ', headinfo.OmegaLambda, ' to ', OmegaLambda
                ...
                if(HubbleParam .ne. -1.) then
                        if(ifile.eq.1) print *, '  ************** change HubbleParam from ', headinfo.HubbleParam , ' to ', HubbleParam 
