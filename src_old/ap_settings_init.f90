...
!									 120.0_dl, 107.0_dl, 100.00_dl, 101.0_dl/)
	...
	! Options of randoms: 2d? 3d? Both? Neither? 
	integer, parameter :: gb_noran = 1, gb_radecran = 2, gb_3dran = 3, gb_radec3dran = 4
	integer :: gb_consider_ran = -1 !!! MUST SET it in main programe!
...
! All variables in the "Adjustable & To be fixed Settings" section must be assigned a value in the main**.f90!!!
	...
	! for data type -1 (xyzw): whether there is the fourth column as weight;	
	logical :: gb_hasweight = .false.
...
105				exit								
  ...
  !------------------------------------------
	subroutine readin_dataran(printinfo)
		...
		if(gb_i_datatype.eq.3) then
			call readin_BOSSdata(printinfo)
		elseif(gb_i_datatype.eq.-1) then
			call readin_xyzwdata(printinfo,gb_hasweight)
		else
		...
		elseif(gb_consider_ran.eq.gb_radec3dran) then
			call readin_HR3ran(printinfo)
			call readin_HR3radecran(printinfo)
		elseif(gb_noran) then
			continue
		else
