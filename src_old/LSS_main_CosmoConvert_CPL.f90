...
implicit none
	...
	integer :: xcol,ycol,zcol,wcol,wacol, maxcol, skiprow,i
	integer, parameter :: bignum = 50000000
	real(dl) :: omin,win,omout,wout,wain,waout, x,y,z,weight,r,redshift, randreds(2,bignum),rat, tmp(1000),&
	...
	printstr = 'Usage: EXE -input intpufilename -omin -omin -win -win -wain wain'//&
		'-omout -omout -wout -wout -waout waout -xcol xcol -ycol ycol -zcol zcol '//&
	...
	wain = 0.00; waout = 0.0;
	hasweight = .false.
	xcol=1; ycol=2; zcol=3; wcol=4; wacol=5;
	skiprow=0;
	...
	do i = 1, numarg
		...
		elseif(trim(adjustl(tmpstr1)).eq.'-win') then
			read(tmpstr2,*) win
		elseif(trim(adjustl(tmpstr1)).eq.'-wain') then
			read(tmpstr2,*) wain
		elseif(trim(adjustl(tmpstr1)).eq.'-omout') then
			read(tmpstr2,*) omout
		elseif(trim(adjustl(tmpstr1)).eq.'-wout') then
			read(tmpstr2,*) wout
		elseif(trim(adjustl(tmpstr1)).eq.'-waout') then
			read(tmpstr2,*) waout
...
!End Test        
	...
	write(*,'(A,2f16.7)') 	'   omin, win, wain = ', real(omin), real(win), real(wain)
	write(*,'(A,2f16.7)') 	'   omout, wout, waout = ', real(omout), real(wout), real(waout)
	...
	write(100,'(A,2f16.7)') 	'   omin, win, wain = ', real(omin), real(win), real(wain)
	write(100,'(A,2f16.7)') 	'   omout, waout = ', real(omout), real(wout), real(waout)
