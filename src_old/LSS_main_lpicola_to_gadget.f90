...
implicit none
	...
	printstr = "Usage: LSS_lpicola_to_gadget   -input inputfile   -output outputname   -xyz_rescale xyz_rescale   -omegam omegam   -omegal omegal   -hubble hubble   -boxsize boxsize   -parmass parmass   -randrat the_rate_you_want_to_output   -xcut_min xcut_min   -xcut_max xcut_max   -ycut_min ycut_min   -ycut_max ycut_max   -zcut_min zcut_min   -zcut_max zcut_max   -write_an_ascii_copy T/F   -just_output_head F #### Example: LSS_gadget_to_fmt3 -input snp04000e.\? -xyz_rescale 0.001 -xyzcut_min 0 -xyzcut_max 100 -write_an_ascii_copy F"
        numarg = iargc()
	if(numarg.le.1) then
		write(*,'(A)') printstr
		stop
	...
	enddo
        ...
        if(omegam<0. .or. boxsize<0. .or. hubble<0. .or. parmass<0. .or. omegal<0.) then
                print *, '(lpicola_to_gadget) ERROR! need value of omegam, omegal, boxsize, hubble, parmass!:', omegam, omegal, boxsize, hubble, parmass
                write(*,'(A)') trim(adjustl(printstr))
                stop
        endif
...
100             exit
        ...
        write(*,'(A)') '   staring write particles. format = block+[x,y,z,...]+block, block+[vx,vy,vz,...]+block, block+[ids]+block ...'
