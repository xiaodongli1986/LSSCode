...
implicit none
	...
	character(len=2000) :: inputfilename, outputfilename, outputfilemassedges, &
	   outputfilename1, outputfilename2, printstr, tmpstr1,tmpstr2, degraded_filename
        real(dl) :: Vtot
	...
	printstr = "Usage: EXE -input intpufilename -rmin rmin "//&
		...
		'-dodegrade dodegrade -degradedfile degraded_filename -numdegrade numdegrade '//&
		'-frac_surface frac_surface -const_nbar_degrade False'//&
	...
	do i = 1, numrbin
		...
		do j = 1, nummassbin
			vol = 4.0/3.0*const_pi * (redges(i+1)**3.0-redges(i)**3.0) * frac_surface
        		    ...
        		    write(*,'(2f15.7,2e15.7, i10, e15.7)') redges(i),redges(i+1),&
			      massedges(j),massedges(j+1),binnednum(i,j), binnednum(i,j)/vol
                        endif
        	...
        	close(3)
                ...
                do i = 1, numrbin
                        ...
                        if(nownum .lt. targetnum(i) ) then
                                write(*,'(A,i12,i12,A,f10.3,A,f10.3)') '   WARNING!!! current < target:: '//&
				  'Find: current/target-#: ', nownum, targetnum(i), ' within ', redges(i), '< r <', redges(i+1)
                                masscuts(i) = 0; cycle
                        endif
        	        do j = nummassbin, 1, -1
...
103     		exit
        	...
        	write(*,'(A,e15.7,A,i10,A,f15.7)') '# We interploate and adopt  mass>masscut=', masscut, ', we obtain ', nlines_degrade, &
        		 ' halos. Rato (num-obtaned/num-goal): ', dble(nlines_degrade)/dble(numdegrade)
        	write(3,'(A,e15.7,A,i10,A,f15.7)') '# We interploate and adopt  mass>masscut=', masscut, &
		         ', we obtain ', nlines_degrade, &
        		 ' halos. Rato (num-obtaned/num-goal): ', &
			 dble(nlines_degrade)/dble(numdegrade)
                do i = 1, numrbin
                        write(*,'(A,i12,i12,i12,A,f10.3,A,f10.3,A,e15.7,A,f10.7)') &
			  '   Total/Target/Final-#: ', sum(binnednum(i,:)), targetnum(i), &
			  countsnum(i), ' within ', redges(i), '< r <', redges(i+1), '; masscut = ', &
			  masscuts(i), '; final/target = ', (countsnum(i)+0.) / (targetnum(i)+0.)
                        write(3,'(A,i12,i12,i12,A,f10.3,A,f10.3,A,e15.7,A,f10.7)') &
			  '   Total/Target/Final-#: ', sum(binnednum(i,:)), targetnum(i), countsnum(i), &
			  ' within ', redges(i), '< r <', redges(i+1), '; masscut = ', masscuts(i), &
			  '; final/target = ', (countsnum(i)+0.) / (targetnum(i)+0.)
                enddo
        	close(3)
                close(1)
        endif
