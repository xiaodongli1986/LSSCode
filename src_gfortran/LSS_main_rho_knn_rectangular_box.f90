...
implicit none
	...
	printstr = " Computing density around each particle. Using k nearest neighbours. Periodical cuboid. \\n"//&
		...
		"-margin 30 -numNB 30 -outputfile yourfilename -xsize 1024 -ysize 1024 -zsize 1024 \\n"//&
	...
	enddo
        ...
        optionstr = ".numNB"//trim(adjustl(tmpstr1))//".margin"//trim(adjustl(tmpstr2))//".xsize"//&
		trim(adjustl(tmpstr3))//".ysize"//trim(adjustl(tmpstr4))//".zsize"//trim(adjustl(tmpstr5))
...
!        write(*,'(A,i10,A,i10,A)') ' (rho_knn_periodic_cuboid) ', npar2, ' particles stored in par2s.'
		...
		' (rho_knn_periodic_cuboid) Finishing rho-estimate for ', npar1, '  particles. ', n1, '(',&
                real(n1)/real(npar1)*100, '%) have   max_dist > margin;  ', n2, '(', real(n2)/real(npar2)*100, &
		'%) have  possible neighbors lying out!'
        close(981)
