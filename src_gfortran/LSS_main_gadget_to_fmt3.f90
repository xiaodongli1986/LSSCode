...
implicit none
	...
	character(len=char_len) :: tmpstr1, tmpstr2, inputfile, outputname, printstr, inputfiles(10000)
	    ...
	    write_an_ascii_copy =.false., write_a_gadget_copy =.false., just_output_head = .false., print_all_head = .false.
	...
	printstr = "Usage: LSS_gadget_to_fmt3 -input inputfile -output outputname -xyz_rescale xyz_rescale "//&
		"-randrat the_rate_you_want_to_output -xcut_min xcut_min -xcut_max xcut_max -ycut_min ycut_min -ycut_max "//&
		"ycut_max -zcut_min zcut_min -zcut_max zcut_max -output_an_ascii_copy/write_an_ascii_copy T/F "//&
		"-write_a_gadget_copy T/F -just_output_head F -print_all_head #### Example: LSS_gadget_to_fmt3 "//&
		"-input snp04000e.\?  -xyz_rescale 0.001 -xyzcut_min 0 -xyzcut_max 100 "//&
		"-write_an_ascii_copy F  ### This is format of l-picola unformatted file; "//&
		"if converting to fmt3, redshift set as -1 (means that there is no redshift; "//&
		"it is a light cone!); we may need to write LSS_fmt4_to_gadget for rockstar halo finder..."
        numarg = iargc()
	if(numarg.le.1) then
		write(*,'(A)') printstr
		stop
	...
	enddo
        ...
        write(*,'(A)') '#################################################'//&
		'#################################################################'
        write(*,'(A)') '(LSS_gadget_to_fmt3) Converting gadget files to block+head+block+blcok+'//&
		'[x,y,z,vx,vy,vz,1.0]*npar+blcok format !'
        ...
        if (just_output_head) then
                write(*,'(A)') '  WARNING!! (LSS_gadget_to_fmt3) Will just output the head! '//&
			'fmt = 6*[double] (noutput, boxsize, mass, redshfit, Omega0, HubbleParam)'
        endif
...
100             exit
        ...
        do ifile = 1, nfile
                ...
                if(ifile .eq. 1) then
                       ...
                       write(*,'("  ",6e25.17)') ntotal+0., headinfo%boxsize, nonzeromass, headinfo%redshift, &
		           headinfo%Omega0, headinfo%HubbleParam
                       write(999,'(6e25.17)') ntotal+0., headinfo%boxsize, nonzeromass, &
		           headinfo%redshift, headinfo%Omega0, headinfo%HubbleParam
                       close(999)
                       if(just_output_head) then
        ...
        if(write_a_gadget_copy) then
                ...
                !write(2001) block0
                write(*,*)
                write(*,'(i15,A,i15,A,f12.6)') noutput, ' particles selected from ', ntotal, &
		    '       rat = ', real(noutput)/real(ntotal)
                write(*,'(A)') '   starting write particles. format = block+headinfo+block+block+'//&
			'[x,y,z]*noutput+block+block+[vx,vy,vz]*noutput+block ...'
                !blockg = noutput*12
                !write(2001) blockg
        ...
        if(write_an_ascii_copy) then
                ...
                write(*,*)
                write(*,'(i15,A,i15,A,f12.6)') noutput, ' particles selected from ', &
		     ntotal, '       rat = ', real(noutput)/real(ntotal)
                do i = 1, noutput
