...
contains
	...
	subroutine read_in_npar(basename, istep, num_threads, npar)
                ...
                if(num_threads.eq.0) then
                        ...
                        !open(file=trim(adjustl(filenamenpar)),unit=1000,action='read',form='binary',access='stream'); read(1000) npar; close(1000) ! gfortran fmt
                        ...
                        do ifile = 0, num_threads-1
                          ...
                          !open(file=trim(adjustl(filenamenpar)),unit=1000,action='read',form='binary',access='stream'); read(1000) npar_tmp; close(1000) ! gfortran fmt
                          open(file=trim(adjustl(filenamenpar)),unit=1000,action='read',form='unformatted',access='stream'); 
	...
	implicit none
                ...
                if(num_threads.eq.0) then
                        ...
                        do ifile = 0, num_threads-1
                                ...
                                filename_tmp = trim(adjustl(filename))//"_thread"//trim(adjustl(tmpstr))
                                filenamenpar = trim(adjustl(filename_tmp))//".npar"
                                !open(file=trim(adjustl(filenamenpar)),unit=1000, action='read', form='binary', access='stream') ! gfortran fmt
                                open(file=trim(adjustl(filenamenpar)),unit=1000, action='read', form='unformatted', access='stream')
                                read(1000) npar_tmp; close(1000)
                                write(*,'(A,i15,A,10x,A)') '     read-in ',npar_tmp, &
                                        '       particles from ', trim(adjustl(filenamenpar)); 
                                !open(file=trim(adjustl(filename_tmp)),unit=1001, action='read', form='binary', access='stream') ! gfortran fmt
                                open(file=trim(adjustl(filename_tmp)),unit=1001, action='read', form='unformatted', access='stream') 
                                read(1001) rmid
	...
	subroutine remove_some_ids(ids1, ids2, xyzvs1, xyzvs2, rmid1, rmid2, ids1_rlt, ids2_rlt, npar1, npar2, boxsize, test_flag)
		...
		dr1_mean = 0; dr2_mean = 0
                !write(*,'(A,\)') '    (remove_some_ids) start: 0% ' ! gfortran fmt
	...
	!                                        print *, id1, id2 ! hahahahaaha
		                        ...
		                        id1=ids1(i1)
                                        if(mod(i1,ipar_print).eq.0) then
                                                !write(*,'(A,f4.1,A,\)') '==>', i1 / (npar1+0.) * 100, '%' ! gfortran fmt
	...
	subroutine read_cola_lc(filename, ids, xyzvs, npar, rmid)
		...
		!open(file=trim(adjustl(filename2)),unit=1000, action='read', form='binary', access='stream') ! gfortran fmt
		open(file=trim(adjustl(filename2)),unit=1000, action='read', form='unformatted', access='stream')
		read(1000) npar
		close(1000)
		...
		!open(file=trim(adjustl(filename)),unit=1001, action='read', form='binary', access='stream') ! gfortran fmt
		open(file=trim(adjustl(filename)),unit=1001, action='read', form='unformatted', access='stream') ! gfortran fmt
		read(1001) rmid
		do i =1, npar
		        read(1001) ids(i)
		        read(1001) xyzvs(1:6,i)
...
implicit none
	...
	character(len=char_len) :: tmpstr1, tmpstr2, basename, outputfile, &
                printstr, filename, filenamenpar,filename1,filename2, rltfile1,rltfile2, &
                outputname, tmpstr, gadgetfile, cmdstr1, cmdstr2
	integer :: i, istep, istep1, istep2 , status1, status2, access, num_threads = 0, npar_tmp
        ...
        printstr = "Usage: LSS_cola_lc_merge   -basename BASE_FILE_NAME    -istep1 BEGIN_OF_ISTEP    "//&
                "-istep2 END_OF_ISTEP -omegam omegam -h h   -nparticle nparticle    -boxsize BOXSIZE   "//&
                "-num_threads num_threads    -write_an_ascii_copy ... # will output a series of gadget-format files"
	if(iargc().le.1) then
		write(*,'(A)') printstr
		stop
	endif
	...
	enddo
        ...
        write(*, '(A, i4, A, i4, A, i4)') ' (cola_lc_merge) will scan files index from ', &
                istep1, ' to', istep2, '; num_threads = ', num_threads
        ...
        do istep = istep1, istep2-1
                ...
                call remove_some_ids(ids1, ids2, xyzvs1, xyzvs2, rmid1, rmid2, &
                        ids1_rlt, ids2_rlt, npar1, npar2, real(boxsize), test_flag)
                ...
                !open(file=trim(adjustl(rltfile1)),unit=10001, action='write', form = 'binary', access = 'stream') ! gfortran fmt
                !open(file=trim(adjustl(rltfile2)),unit=10002, action='write', form = 'binary', access = 'stream') ! gfortran fmt
                open(file=trim(adjustl(rltfile1)),unit=10001, action='write', form = 'unformatted', access = 'stream')
                open(file=trim(adjustl(rltfile2)),unit=10002, action='write', form = 'unformatted', access = 'stream')
        ...
        do istep = istep1, istep2-1
                ...
                if(status1 .eq. 0) then
                        !open(file=trim(adjustl(rltfile1)),unit=2000, action='read', form='binary', access='stream') ! gfortran fmt
                        open(file=trim(adjustl(rltfile1)),unit=2000, action='read', form='unformatted', access='stream')
                        read(2000) ids_rlt1
!                        print *, '   finishing read in ', trim(adjustl(rltfile1))
                ...
                if(status2 .eq. 0) then
                        !open(file=trim(adjustl(rltfile2)),unit=2000, action='read', form='binary', access='stream') ! gfortran fmt
                        open(file=trim(adjustl(rltfile2)),unit=2000, action='read', form='unformatted', access='stream')
                        read(2000) ids_rlt2
