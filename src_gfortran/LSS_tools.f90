...
contains
  ...
  ! format I/O
  !------------------------------------------
	character(len=char_len) function format_string (n, unit_string, appendstr)
		! Dummy
		integer :: n
		character(len=*) :: unit_string
		character(len=*), optional :: appendstr
		!print *, ''
		if(n>9e8)  then
			print *, 'ERROR (format_string)!: n exceeds maximal value: ', n
		endif
		if(.not.present(appendstr)) then
			write(format_string, '("(", i8, "  (",A,")", ")" )') n, trim(adjustl(unit_string))
		else
			write(format_string, '( "(", i8, "  (",A,"), ", A, " )" ) ') n, trim(adjustl(unit_string)), trim(adjustl(appendstr))
		endif
		!write(format_string, '( i5, A )') n, trim(adjustl(unit_string))
  ...
  !------------------------------------------
	subroutine count_line_number (file_name, line_number)
		...
		open(unit=1456,FILE=file_name,ERR=2,action='read')
...
100		close(1456)
  ...
  !------------------------------------------	
	subroutine read_in (file_name, n_column, n_lines, total_data)
		...
		open(unit=4321,file=file_name,action='read')
		do i = 1, n_lines
...
33		write(*,*) "Error occurs when writing into the file ", file_name
  ...
  ! distance of two vectors
  !------------------------------------------	
	real(dl) function distancexyz(x0,y0,z0, x1,y1,z1)
		! Dummy
		real(dl) :: x0,y0,z0, x1,y1,z1
		!local
		integer :: i		

		distancexyz = (x1-x0)**2.0 + (y1-y0)**2.0 + (z1-z0)**2.0
		distancexyz = sqrt(distancexyz)
	end function distancexyz
  ...
  ! Volume of sphere
  !---------------------------------------------------------------
	real(dl) function sphere_vol(r)
		real(dl), intent(in) :: r
		sphere_vol = 4.0 * const_pi / 3.0 * r**3.0
	end function sphere_vol
...
! Find out index; start from 1
                subroutine xi3d(x,y,z,ix,iy,iz,xyzmin,xyzmax,ncell)
                ! dummy
                real(dl), intent(in) :: x,y,z, xyzmin,xyzmax
                integer, intent(out) :: ix,iy,iz
                integer, intent(in) :: ncell
                ! local
                real(dl) :: delta
                
                delta = (xyzmax-xyzmin) / dble(ncell)
                ix = int((x-xyzmin)/delta + 1.0)
                iy = int((y-xyzmin)/delta + 1.0)
                iz = int((z-xyzmin)/delta + 1.0)
                if(x<xyzmin.or.x>xyzmax.or.y<xyzmin.or.y>xyzmax.or.z<xyzmin.or.z>xyzmax) then
                        print *, 'xi3d: outflow!: ', x,y,z,xyzmin,xyzmax
                        ix = min(ix,ncell); ix = max(ix,1)
                        iy = min(iy,ncell); iy = max(iy,1)
                        iz = min(iz,ncell); iz = max(iz,1)
                endif

        end subroutine xi3d

        integer function logmi(m, mmin, mmax, ncell)
                ! dummy
                real(dl), intent(in) :: m, mmin,mmax
                integer, intent(in) :: ncell
                logmi = int((log10(m) - log10(mmin)) / &
                ((log10(mmax)-log10(mmin))/dble(ncell))+1.0)
                if(m<mmin.or.m>mmax) then
                        print *, 'logmi: outflow!: ', m, mmin, mmax
                        logmi = min(logmi,ncell)
                        logmi = max(logmi,1)
                endif
