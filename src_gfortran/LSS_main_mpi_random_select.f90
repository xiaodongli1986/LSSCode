...
implicit none
	...
	call mpi_comm_rank(mpi_comm_world,myid,ierr)
        ...
        printstr = "Randomly select a part of the input file. You must have "//&
		"the files stored in file:  (by default LSS_mpi_random_select.filelist)    "//&
		"Usage: LSS_mpi_random_select -inputfile inputfilename -randrat your_rat -suffixstr your_suffix -use_filelist True"
...
100     close(10000)
        ...
        do ifile = myid+1, nfile, nproc
                ...
                enddo
101             close(10000)
                write(*, '(A,i4,i4,";",i12,A,A,";",i12,A,A)')'myid, nproc = ', myid, nproc, &
			iline, ' lines read from ', trim(adjustl(file1)), iwriteline, ' lines write to ', trim(adjustl(file2))
        enddo
