...
100                     exit
           ...
           write(*,'(A,i15,A,A)') ' Finishing particle output (stage 1). In total we have ', &
           ...
           if(block_write)then
			...
			trim(adjustl(inputfiles(ifile))), ' for block read-in...', ifile, nfile
             ...
             enddo !endodicycle
             do i1 = 1,nbox; do i2=1,nbox; do i3=1,nbox
                        write(*,'(A,i3,i3,i3, A,i10,i10,A,i10,i10)') '  Finishing all_data storage: i1,i2,i3 = ', &
...
200     allocate(tmp_xyzvs(7,n_par_random_output))
        ...
        write(*,'(A)') '   staring write particles to '//trim(adjustl(par_random_output_file))//&
