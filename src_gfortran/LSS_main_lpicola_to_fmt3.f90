...
implicit none
	...
	character(len=char_len) :: tmpstr1, tmpstr2, inputfile, outputname, printstr, inputfiles(10000), headfile
        ...
        logical :: do_xcut1, do_ycut1, do_zcut1, do_xcut2, &
		do_ycut2, do_zcut2, write_an_ascii_copy =.false., just_output_head = .false.
        double precision :: tmpdoubles(64), nonzeromass=0., randrat = 10., randx, boxsize, omegam, hubble, parmass


	
	printstr = "Usage: LSS_lpicola_to_fmt3 -input inputfile -output outputname -xyz_rescale "//&
	   "xyz_rescale -omegam omegam -w w -hubble hubble -boxsize boxsize "//&
	   "-parmass parmass   -randrat the_rate_you_want_to_output -xcut_min xcut_min -xcut_max xcut_max -ycut_min "//&
	   "ycut_min -ycut_max ycut_max -zcut_min zcut_min -zcut_max zcut_max -write_an_ascii_copy T/F "//&
	   "-just_output_head F -headfile T #### Example: LSS_gadget_to_fmt3 -input snp04000e.\? "//&
	   "-xyz_rescale 0.001 -xyzcut_min 0 -xyzcut_max 100 -write_an_ascii_copy F"
        numarg = iargc()
	if(numarg.le.1) then
		write(*,'(A)') printstr
		stop
	...
	enddo
        ...
        if(trim(adjustl(headfile)).ne.'') then
                print *
                write(*,'(A)') ' Read in parameters from lpicola headfile:  '//&
			'(This will ignore hand-given values of omegam, w, boxsize, parmass, hubble!)'
