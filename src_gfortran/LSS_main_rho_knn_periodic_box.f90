...
subroutine periodic_shift_ps(x,y,z, boxsize, ps)
        real(dl) :: x,y,z, boxsize, ps(3,26)
        integer :: i 
        i = 1
        ps(1,i) = x+boxsize; ps(2,i) = y; ps(3,i) = z; i=i+1
        ps(1,i) = x; ps(2,i) = y+boxsize; ps(3,i) = z; i=i+1
        ps(1,i) = x; ps(2,i) = y; ps(3,i) = z+boxsize; i=i+1

        ps(1,i) = x-boxsize; ps(2,i) = y; ps(3,i) = z; i=i+1
        ps(1,i) = x; ps(2,i) = y-boxsize; ps(3,i) = z; i=i+1
        ps(1,i) = x; ps(2,i) = y; ps(3,i) = z-boxsize; i=i+1

        ps(1,i) = x+boxsize; ps(2,i) = y+boxsize; ps(3,i) = z; i=i+1
        ps(1,i) = x; ps(2,i) = y+boxsize; ps(3,i) = z+boxsize; i=i+1
        ps(1,i) = x+boxsize; ps(2,i) = y; ps(3,i) = z+boxsize; i=i+1
        ...
        ps(1,i) = x+boxsize; ps(2,i) = y+boxsize; ps(3,i) = z+boxsize; i=i+1
        ps(1,i) = x-boxsize; ps(2,i) = y+boxsize; ps(3,i) = z+boxsize; i=i+1
        ps(1,i) = x+boxsize; ps(2,i) = y-boxsize; ps(3,i) = z+boxsize; i=i+1
        ps(1,i) = x+boxsize; ps(2,i) = y+boxsize; ps(3,i) = z-boxsize; i=i+1
        ps(1,i) = x-boxsize; ps(2,i) = y-boxsize; ps(3,i) = z+boxsize; i=i+1
...
subroutine ps_within_margin(ps1, n1, ps2, n2, boxsize, margin)
        integer :: i, n1, n2
        real(dl) :: ps1(3,n1), ps2(3,n1), boxsize, margin, x,y,z
        n2 = 1
        do i = 1, n1
                x=ps1(1,i); y=ps1(2,i); z=ps1(3,i)
                if(-margin<x .and. x<boxsize+margin .and. -margin<y .and. &
                         y<boxsize+margin .and. -margin<z .and. z<boxsize+margin) then
...
program main_rho_knn_periodic_box
...
implicit none
	...
	character(len=char_len) :: inputfile, outputfile, printstr, optionstr, tmpstr1, tmpstr2, tmpstr3, tmpstr4
	...
	printstr = " Computing density around each particle. Using k nearest neighbours. Periodic Bounday Box. \\n"//&
                "      Usage::: EXE -inputfile inputfile -xcol 1 -ycol 2 -zcol 3"//&
		" -margin 30 -numNB 30 -outputfile yourfilename -boxsize 1024  \\n"//&
	...
	enddo
        ...
        write(tmpstr3, '(f15.2)') boxsize
        optionstr = ".numNB"//trim(adjustl(tmpstr1))//".margin"//trim(adjustl(tmpstr2))//".boxsize"//trim(adjustl(tmpstr3))
        write(*,'(A,A)') ' (rho_knn_periodic_box) options: ', trim(adjustl(optionstr))
	if(trim(adjustl(outputfile)).eq."") then
                outputfile = trim(adjustl(inputfile))//trim(adjustl(optionstr))
	endif
        write(*,'(A,A)') ' (rho_knn_periodic_box) inputfile: ', trim(adjustl(inputfile))
        write(*,'(A,A)') ' (rho_knn_periodic_box) outputfile: ', trim(adjustl(outputfile))
...
!        write(*,'(A,i10,A,i10,A)') ' (rho_knn_periodic_box) ', npar2, ' particles stored in par2s.'



        ! build the grid for NNB search...
        write(*,'(A,i10,A,i10,A)') ' (rho_knn_periodic_box) start to building cells...'
        ...
        do i = 1, gb_numdata
                ...
                if(compute_list(i)) then
                        ...
                        if(x-max_dist < -margin .or. x+max_dist > boxsize+margin .or.y-max_dist < -margin .or. y+max_dist > &
                               boxsize+margin .or. z-max_dist < -margin .or. z+max_dist > boxsize+margin) n2=n2+1
                        npar1 = npar1 + 1
                        write(981,'(8e15.7)') x,y,z,rho,drhox,drhoy,drhoz,max_dist
                endif
        enddo
        print *
        write(*,'(A,i10,A,i10,A,f5.2,A,i10,A,f5.2,A)') &
		' (rho_knn_periodic_box) Finishing rho-estimate for ', npar1, '  particles. ', n1, '(',&
                real(n1)/real(npar1)*100, '%) have   max_dist > margin;  ', n2, &
		'(', real(n2)/real(npar2)*100, '%) have  possible neighbors lying out!'
        close(981)



        write(*,'(A)') ' (rho_knn_periodic_box) Done.'
