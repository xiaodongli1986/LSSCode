...
implicit none
	...
	do while(.true.)
		...
		if(.not.addw1) then
			!write(2,'(<maxcol>(e15.7,1x))') ra, dec, redshift, tmp(4:maxcol) ! gfortran fmt
			write(2,format_string(maxcol,'(e15.7,1x)')) ra, dec, redshift, tmp(4:maxcol) ! gfortran fmt
		else
			!write(2,'(<maxcol>(e15.7,1x)," 1")') ra, dec, redshift, tmp(4:maxcol) ! gfortran fmt warning!
			write(2,format_string(maxcol, '(e15.7,1x)', '" 1"')) ra, dec, redshift, tmp(4:maxcol) 
		endif
		cycle
101		exit
	enddo
