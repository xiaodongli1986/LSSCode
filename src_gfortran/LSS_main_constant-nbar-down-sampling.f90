...
implicit none
	...
	printstr = 'Usage:  EXE -input inputfilename -xcol xcol -ycol ycol -zcol zcol '//&
		'-masscol masscol -deltar deltar -frac_surface frac_surface -deltar deltar -suffix '//&
		'suffix -nbar nbar -rmin rmin -rmax rmax ### if all-sky sample then frac_surfact=1 if half-sky then 0.5 by default 1 ###'
