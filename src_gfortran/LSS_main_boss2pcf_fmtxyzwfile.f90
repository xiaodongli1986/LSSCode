...
implicit none
	...
	! gfortran fmt
	printstr = "EXE -datatype datatype -inputfile inputfile [-outputfile outputfile] '//&
	  '[-ignorecp ignorecp] [-ignoremask ignoremask] ### if ignorecp, will not '//&
	  'take CP into consideration; only applicable for mock or mockran; by default ignorecp=False"
	if(iargc().le.1) then
		print *, printstr
		stop
	endif
	...
	elseif(trim(adjustl(datatype)).eq.'compact') then
		...
		write(*,'(A,A)') ' ERROR (LSS_boss2pcf_fmtxyzwfile): wrong datatype: must '//&
			'be data, dataran, mock or mockran!', trim(adjustl(datatype))
		stop
	endif

	if(ignorecp .and. (dt.eq.1 .or. dt.eq.2 .or. dt.eq.5)) then
		write(*,'(A,A)') ' ERROR (LSS_boss2pcf_fmtxyzwfile): ignorecp only possible '//&
			'when datatype is mock, mockran!: ', trim(adjustl(datatype))
		stop
	endif
	if(ignoremask .and. (dt.eq.1 .or. dt.eq.2 .or. dt.eq.5)) then
		write(*,'(A,A)') ' ERROR (LSS_boss2pcf_fmtxyzwfile): ignoremask only possible when datatype '//&
			'is mock, mockran!: ', trim(adjustl(datatype))
		stop
	endif
