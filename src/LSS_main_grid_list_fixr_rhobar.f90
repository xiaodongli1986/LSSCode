...
implicit none
	...
	outputfmtstr = '# result using fixed distance '//trim(adjustl(fixrstr))//'. fmt:   x,y,z,  numNB (number of neighbour found),  touchboundary (if True, the sphere near this point touches the boundary, results may be not reliable),   rhomean (mean density within the volume),   rho,drhox,drhoy,drhoz (density and gradients estimated using spline kernel),   nbarmean,nbar,dnbarx,dnbary,dnbarz (same as rho but number density)' 
