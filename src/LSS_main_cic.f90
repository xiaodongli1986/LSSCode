...
100             exit
        ...
        if(nsplit.eq.1) then
          ...
          do k=1,nsplit
             ...
             open(file=trim(adjustl(outputname))//'.ifile'//trim(adjustl(tmpstr1))//'.rhogrid',unit=2001,form='unformatted',action='write')
             open(file=trim(adjustl(outputname))//'.ifile'//trim(adjustl(tmpstr1))//'.pxgrid',unit=2002,form='unformatted',action='write')
             open(file=trim(adjustl(outputname))//'.ifile'//trim(adjustl(tmpstr1))//'.pygrid',unit=2003,form='unformatted',action='write')
             open(file=trim(adjustl(outputname))//'.ifile'//trim(adjustl(tmpstr1))//'.pzgrid',unit=2004,form='unformatted',action='write')
